...
from Source.constants import TILE_SIZE, TILE_MAP
from Source.message import ToSay, Refuse, Message


class Character(Body):
    def __init__(self, images: tuple[pygame.Surface, ...], health: int, move_speed: int):
...
class Player(Character):
    ...
    def actions(self, event: pygame.event.Event, around: list[pygame.rect.Rect, ...]) -> None | Message:
        ...
        if event.key == pygame.K_a:
            ...
            if not left_tile.can_collide:
                self._rect_.x -= self._move_speed_ * TILE_SIZE
            ...
            if not top_tile.can_collide:
                self._rect_.y -= self._move_speed_ * TILE_SIZE

...
class Game:
    def __init__(self):
        ...
        self._map_ = Map(self._player_.coordinates)

        self._tile_map_ = self._map_.map(self._player_.coordinates, self._current_level_)
    ...
    def draw_tiles(self) -> None:
        x, y = self._tile_map_.shape

        for i in range(x):
            for j in range(y):
                coordinates = (i * TILE_SIZE, j * TILE_SIZE)

                current_tile = self._tile_map_[i][j]
    ...
    def around_character(self, character: Character) -> list[numpy.ndarray[Any, numpy.dtype[Any]]]:
        x, y = character.coordinates // TILE_SIZE

        return [self._tile_map_[i][j] for i, j in [(x + 1, y), (x, y - 1), (x - 1, y), (x, y + 1)]]
    ...
    def init_player() -> Player:
        ...
        x, y = DISPLAY_SIZE[0], DISPLAY_SIZE[1]

        player.rect = random.randint(5, x - 5), random.randint(5, y - 5)
